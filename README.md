...
- Вывод: **интерактивная схема** прямо в браузере — собственный SVG-рендер с
  зумом, панорамой и фокусом; экспорт **SVG**, **PNG**, **Mermaid** и **Graphviz `.dot`**.
...
Бинарник самодостаточный: страница со стилями и скриптом вшита через `go:embed`
и не тянет внешних ресурсов (ни CDN, ни шрифтов) — интернет для работы не нужен.
...
distroless (вся статика вшита в бинарник через `go:embed`, внешних зависимостей нет).
...
| `render/mermaid` | Граф → Mermaid (текст для экспорта) |
