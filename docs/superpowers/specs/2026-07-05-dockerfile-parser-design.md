...
**Дата:** 2026-07-05
**Тип:** производственная практика

## Цель

Веб-сервис, который парсит `docker-compose.yaml` (позже — Helm-чарты) и строит наглядную
схему зависимостей: какой сервис с каким общается по сети. Новый сотрудник, глядя на схему,
за 5 минут понимает, с какими компонентами общается выбранное приложение.

## Решения

| Вопрос | Решение |
|--------|---------|
| Язык / стек | **Go** (stdlib: `net/http`, `html/template`, `embed`; `gopkg.in/yaml.v3`; без веб-фреймворка) |
| Форма | **Веб-сервис**: HTTP API + страница, граф рисуется Mermaid.js в браузере |
| Входные форматы | Сначала **docker-compose**, затем **Helm** (фаза 2) |
...
| Основной вывод | **Mermaid** (в браузере); `.dot` (Graphviz) — бонус-экспорт |
...
- **`parser/compose`** — docker-compose → `model.Graph` (фаза 1).
- **`parser/helm`** — Helm → `model.Graph` (фаза 2, тот же интерфейс).
...
- **`config`** — паттерны внешних сервисов, игнор-лист.
...
type Node struct {
    ...
    Kind     string   `json:"kind"`     // "service" | "database" | "external"
...
- **Фокус** (под требование «за 5 минут понять, с чем общается приложение»): выпадающий список
  сервисов; выбор подсвечивает узел и соседей, остальное приглушает; переключатель «только
  соседи» (1 хоп). Кнопки «скопировать Mermaid» и «скачать .dot».
...
Best-effort; детали уточняются на этапе реализации фазы 2.
...
- **Фаза 1 (ядро, рабочее демо):** model → parser/compose → analyze → render/mermaid+dot →
  web со страницей и фокусом. Полностью рабочий веб-сервис для docker-compose.
- **Фаза 2:** parser/helm.
- **Фаза 3 (если останется время):** конфиг паттернов в UI, экспорт PNG.
